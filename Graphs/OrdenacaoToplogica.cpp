...
void adicionarAresta(Grafo* grafo, int origem, int destino) {
    ...
    // Se for grafo direcionado, remova este trecho
    /*
    novoNo = new No();
    novoNo->valor = origem;
...
void dfs_visit(Grafo* grafo, int u, vector<int>& lista) {
    cor[u] = CINZA;
    tempo++;
    desc[u] = tempo;
    ...
    lista.push_back(u); // Armazena o vértice finalizado
}

void dfs(Grafo* grafo, vector<int>& lista) {
...
void ordenacaoTopologica(Grafo* grafo) {
    vector<int> lista;  // Lista para armazenar ordem de finalização
    dfs(grafo, lista);

    // Inverter a lista para obter a ordenação topológica
